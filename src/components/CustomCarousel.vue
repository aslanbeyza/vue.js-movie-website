...
export default {
  ...
  methods: {
    getRandomMovies() {
      ...
      return shuffledMovies.slice(0, 10); // Gösterilecek film sayısını ayarla
    },
    startAutoScroll() {
      this.stopAutoScroll(); // Çakışmayı önlemek için durdur
    ...
    scrollRight() {
      ...
      if (container) {
        container.scrollBy({
          right: 300, // Bir sonraki filme git
          behavior: "smooth",
        });
      } else {
        console.error("Carousel container is undefined.");
